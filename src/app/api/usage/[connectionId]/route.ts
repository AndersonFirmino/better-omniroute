...
export async function GET(
  ...
  try {
    ...
    // Only OAuth connections and providers with dedicated quota APIs have usage data
    const providersWithApiKeyUsage = new Set(["glm"]);
    if (connection.authType !== "oauth" && !providersWithApiKeyUsage.has(connection.provider)) {
      return Response.json({ message: "Usage not available for API key connections" });
    }

    // Refresh credentials if needed using executor (OAuth only)
    let refreshed = false;
