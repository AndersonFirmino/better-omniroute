...
export function getDbInstance(): SqliteDatabase {
  ...
  // Auto-seed 003 as applied when the "group" column already exists.
  // ensureProviderConnectionsColumns() adds it before runMigrations() runs,
  // so the migration runner would fail with "duplicate column name: group".
  const providerCols = (
    db.prepare("PRAGMA table_info(provider_connections)").all() as Array<{ name?: string }>
  ).map((c) => String(c.name ?? ""));
  if (providerCols.includes("group")) {
