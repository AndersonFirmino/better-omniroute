...
export async function handleChat(request: any, clientRawRequest: any = null) {
  ...
  // Auto-resolve: when API key has autoResolve enabled, resolve bare model
  // shorthands (e.g. "sonnet", "haiku", "opus") to their Claude equivalents.
  // This prevents them from falling through to OpenAI as unknown models.
  if (apiKeyInfo?.autoResolve && modelStr && !modelStr.includes("/")) {
    const CLAUDE_SHORTHANDS: Record<string, string> = {
      sonnet: "claude-sonnet-4-6",
      haiku: "claude-haiku-4-5-20251001",
      opus: "claude-opus-4-6",
    };
    const resolved = CLAUDE_SHORTHANDS[modelStr.toLowerCase()];
    if (resolved) {
      log.info("AUTO-RESOLVE", `${modelStr} → claude/${resolved}`);
      modelStr = `claude/${resolved}`;
