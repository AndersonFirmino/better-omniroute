 ...
 * @param {string[]|null} allowedConnections - If non-empty, restrict to these connection IDs only
 */
export async function getProviderCredentials(
  ...
  try {
    ...
    // If allowedConnections is non-empty, restrict to only those connection IDs
    const availableConnections = connections.filter((c) => {
      ...
      if (allowedConnections && allowedConnections.length > 0) {
        if (!allowedConnections.includes(c.id)) return false;
      }
      return true;
    });
 ...
 * Supports both OpenAI format (Authorization: Bearer) and
 * Anthropic/Claude format (x-api-key header).
 */
