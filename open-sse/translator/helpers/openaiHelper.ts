...
// Recursively ensure every "type":"object" schema has a "properties" field.
// OpenAI rejects schemas missing it; Anthropic/Pydantic may omit it.
function sanitizeSchema(schema: any): any {
  ...
  for (const key of ["properties", "items", "additionalProperties"]) {
    if (out[key] && typeof out[key] === "object") {
      out[key] = sanitizeSchema(out[key]);
    }
  }
  ...
  if (out.definitions) {
    out.definitions = Object.fromEntries(
      Object.entries(out.definitions).map(([k, v]) => [k, sanitizeSchema(v)])
    );
  }
  return out;
}
        ...
        if (tool.name && (tool.input_schema || tool.description)) {
          return {
            ...
            function: {
              ...
              parameters: sanitizeSchema(tool.input_schema || { type: "object", properties: {} }),
            },
          };
        }
