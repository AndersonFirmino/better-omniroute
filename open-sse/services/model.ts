...
export async function getModelInfoCore(modelStr, aliasesOrGetter) {
  ...
  if (nonOpenAIProviders.length > 1) {
    // Auto-disambiguate: prefer "claude" provider for Claude models
    if (nonOpenAIProviders.includes("claude")) {
      const canonicalModel = resolveProviderModelAlias("claude", modelId);
      return { provider: "claude", model: canonicalModel, disambiguated: true };
    }
