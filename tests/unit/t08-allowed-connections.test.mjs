...
// ══════════════════════════════════════════════════════════════════
// Bloco 3 — Sidebar + i18n (hardening estático)
// ══════════════════════════════════════════════════════════════════

test("3.1 — Sidebar.tsx NÃO contém /dashboard/auto-combo (feature desabilitada)", () => {
  const sidebarPath = path.join(ROOT_DIR, "src/shared/components/Sidebar.tsx");
  const content = fs.readFileSync(sidebarPath, "utf-8");
  assert.ok(
    !content.includes("/dashboard/auto-combo"),
    "auto-combo should be removed from Sidebar.tsx"
  );
});

test("3.2 — Sidebar.tsx NÃO contém autoCombo como i18nKey (feature desabilitada)", () => {
  const sidebarPath = path.join(ROOT_DIR, "src/shared/components/Sidebar.tsx");
  const content = fs.readFileSync(sidebarPath, "utf-8");
  assert.ok(
    !content.includes('i18nKey: "autoCombo"') && !content.includes("i18nKey: 'autoCombo'"),
    "autoCombo i18nKey should be removed from Sidebar.tsx"
  );
});

test("3.3 — todos os 30 arquivos i18n têm sidebar.autoCombo definido", () => {
  const messagesDir = path.join(ROOT_DIR, "src/i18n/messages");
  const files = fs.readdirSync(messagesDir).filter((f) => f.endsWith(".json"));

  assert.equal(files.length, 30, `Expected 30 i18n files, found ${files.length}`);

  const missing = [];
  for (const file of files) {
    const raw = fs.readFileSync(path.join(messagesDir, file), "utf-8");
    const data = JSON.parse(raw);
    if (!data?.sidebar?.autoCombo) {
      missing.push(file);
    }
  }

  assert.deepEqual(missing, [], `Missing sidebar.autoCombo in: ${missing.join(", ")}`);
});
